...
class Quarter:
  ...
  def next(self,radius):
    ...
    self._center[self._number % 2] += distance_values[self._number]
    self._number = (self._number + 1) % 4

def num_points_get(density,radius):
  num_points = int(math.ceil(density * radius * 0.5 * math.pi))
...
def quarter_circle_add(x_vec,y_vec,offset,center,start_angle,end_angle,radius,
                       num_points):
  angles = np.linspace(math.radians(start_angle),math.radians(end_angle),
                       num=num_points)
